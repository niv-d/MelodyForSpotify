...
struct SpotifyWebView: UIViewRepresentable {
  ...
  func makeUIView(context: Context) -> WKWebView {
    ...
    configuration.userContentController.add(context.coordinator as WKScriptMessageHandler, name: "consoleLog")
  ...
  class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
      ...
      readPageContent { content, error in
        if(error){
          
        }else{
          if(content?.contains("Log into Spotify")){
